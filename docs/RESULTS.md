...
Generated by `task charts` from the csvs (`bench/charts.py`); the interactive version with both
themes and hover values is `docs/charts.html`. Loki cluster stops at 500 rps on purpose: its
higher steps ran into the host swap and are not measurements. The hollow marker is the ELK
2000 rps step that was cut short.

| cpu | memory | disk writes |
|---|---|---|
| ![](charts/cpu-single.svg) | ![](charts/mem-single.svg) | ![](charts/disk-single.svg) |
| ![](charts/cpu-cluster.svg) | ![](charts/mem-cluster.svg) | ![](charts/disk-cluster.svg) |

Bytes per stored line on disk after a full run (wal and caches included):
